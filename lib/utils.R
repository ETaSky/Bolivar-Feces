...
compare_group_dist <- function(dist_mat, metadata, key, by){
    ...
    dist_pairs <- dist_pairs %>% as.matrix()
    for (row in 1:nrow(dist_pairs)){
        dist_pairs[row, 4] = paste(c(get_group_name(dist_pairs[row, 1], metadata, key, by), get_group_name(dist_pairs[row, 2], metadata, key, by)) %>% sort, collapse = "-")
    }
